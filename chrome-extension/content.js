...
console.log('AI Makeover API URL:', CONFIG.API_BASE_URL);
    ...
    async function handleGenerate(magicPrompt, overridePrompt = null) {
        ...
        try {
            ...
            if (data.aiUrl) {
                showComparisonOverlay(wrapper, img.src, data.aiUrl, data.products, () => {
                    // Regenerate callback
                    handleGenerate(magicPrompt, overridePrompt);
                });
            } else {
                showError(wrapper, 'AI Generation Failed: ' + (data.error || 'Unknown error'), btn);
            }
...
function showComparisonOverlay(wrapper, originalUrl, aiUrl, products = [], onRegenerate = null) {
    const host = createElement('div', 'ai-makeover-shadow-host');
    host.style.position = 'absolute';
    host.style.top = '0';
    host.style.left = '0';
        ...
        <img src="${aiUrl}" class="ai-makeover-img-ai ai-makeover-overlay-ai" style="opacity: 1">
            ...
            <div class="ai-makeover-slider-container">
                ...
                <input type="range" min="0" max="100" value="100" class="ai-makeover-range">
                    ...
                    <!-- Regenerate Button -->
                    <button class="ai-makeover-regenerate-btn" title="Regenerate">
                         <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" style="width: 20px; height: 20px;">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" />
                        </svg>
                    </button>
                    <button class="ai-makeover-close-btn" title="Close">
    ...
    const regenBtn = overlay.querySelector('.ai-makeover-regenerate-btn');
    ...
    if (products && products.length > 0) {
        const AMZN_TAG = 'reimagine-20';

        // Create Shop Button
        const shopBtn = createElement('button', 'ai-makeover-shop-btn');
        shopBtn.innerHTML = `
        ...
        // Product Container (Hidden initially)
        const productContainer = createElement('div', 'ai-makeover-products-container');
        productContainer.style.display = 'none';

        // Add products
        products.forEach(prod => {
            const query = encodeURIComponent(prod.query);
            const url = `https://www.amazon.com/s?k=${query}&tag=${AMZN_TAG}`;

            const chip = createElement('a', 'ai-makeover-product-chip');
            chip.href = url;
            chip.target = '_blank';
            chip.title = `Search Amazon for ${prod.label}`;
            chip.innerHTML = `
                ...
                <span class="ai-makeover-chip-text">${prod.label}</span>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" style="width:14px;height:14px;opacity:0.5;margin-left:auto;">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                </svg>
            `;
            // Stop propagation to prevent closing overlay when clicking link
        ...
        shopBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            isOpen = !isOpen;
            productContainer.style.display = isOpen ? 'flex' : 'none';
            shopBtn.innerHTML = isOpen ?
                `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" style="width:18px;height:18px;"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg> Close` :
                `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" style="width:18px;height:18px;"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg> Shop This Look`;
        });
    ...
    if (regenBtn) {
        if (onRegenerate) {
            regenBtn.addEventListener('click', (e) => {
                ...
                // Show the loading wand immediately behind the fading overlay
